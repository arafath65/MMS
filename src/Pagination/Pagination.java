...
public class Pagination extends JPanel {
    ...
    int radius = 2; // How many pages to show around current
    int start = Math.max(2, current - radius);
    int end = Math.min(max - 1, current + radius);

    // Add left ellipsis
    if (start > 2) {
        items.add(new Page.BreakLabel(start - 1));
    }
    ...
    // Add right ellipsis
    if (end < max - 1) {
        items.add(new Page.BreakLabel(end + 1));
    }
